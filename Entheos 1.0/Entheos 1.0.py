...
with open('jentheos.json', 'r') as f:
    past_data = json.load(f)


# get the data from the "jentheos.json" file
date_list = []
...
def dump_data():
    ...
    filename = Path.cwd() / "jentheos.json"
...
def corr_func():
    float_dataset()
    pear_corr = fdb.corr(method='pearson')
    edf = pear_corr.to_dict()
    filename = Path.cwd() / "corr.json"
    with open(filename, 'w') as f:
        json.dump(edf, f)

    json_file = pd.read_json('corr.json')
...
def excel_export():
    json_file = pd.read_json('jentheos.json')
    file = filedialog.asksaveasfilename(defaultextension='.xlsx', filetypes=[("Excel file (.xlsx)",".xlsx")])
    json_file.to_excel(file)
...
clean_button = Button(tab_1, width=60, background="#dadada",fg="black", font=("Ubuntu Light", "12"), borderwidth=3, padx=25, text="Delete Entry Value", command=clean_func)
...
label_mean4.pack()
label_spacemean = Label(tab_4, height=2, width=40, text="",background="#dadada",fg="black", font=("Ubuntu Light", "16"))
label_spacemean.pack()

corr_button = Button(tab_4,  state='disabled', width=50, text='Export Correlation Excel file', border=40, background="#dadada",fg="black", font=("Ubuntu", "12"), borderwidth=3, command=corr_func)
corr_button.pack()
